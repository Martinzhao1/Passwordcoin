...
#if defined(NDEBUG)
# error "Passwordcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Passwordcoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Passwordcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Passwordcoin: find block wanted by given orphan block
...
{
    int64_t nSubsidy = nCoinAge * (0.005 * COIN);
...
// Passwordcoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Passwordcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Passwordcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Passwordcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Passwordcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Passwordcoin: track money supply and mint amount info
...
// Passwordcoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Passwordcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Passwordcoin: compute stake entropy bit for stake modifier
    ...
    // Passwordcoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Passwordcoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Passwordcoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Passwordcoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Passwordcoin-loadblk");
